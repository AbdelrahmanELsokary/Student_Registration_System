...
export default function StudentList() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredStudents.map((s) => [
      s.name,
      // Translate if known grade, otherwise use raw grade or 'unknown' fallback
      s.grade && gradeOptions.includes(s.grade) ? t(`grades.${s.grade.toLowerCase().replace(/ /g, '')}`) : s.grade ? s.grade : t('grades.unknown'),
      s.guardian_phone,
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto font-sans" dir={isRTL ? 'rtl' : 'ltr'}>
      <h2 className={`text-3xl font-bold text-gray-800 mb-6 border-b-2 pb-3 ${isRTL ? 'text-right' : 'text-left'}`}>{t('studentsTable.totalStudents')}</h2>
      ...
      {/* Changed sm:space-x-2 and sm:space-x-reverse to gap-3 for consistent spacing */}
      <div className={`mb-4 flex flex-col sm:flex-row sm:items-center gap-3 space-y-2 sm:space-y-0`}>
        ...
        {/* Add Student Button */}
        <Link
          to="/add-student" // Changed route to /add-student for clarity
        ...
        >
          {isRTL ? (
            <>
              {t('studentsTable.addStudent')} <FaPlus className="mr-2" /> {/* Icon on right for RTL */}
            </>
          ) : (
            <>
        ...
        >
          {isRTL ? (
            <>
              {t('studentsTable.refresh')} <FaSyncAlt className={`mr-2 ${isLoadingRefresh ? 'animate-spin' : ''}`} /> {/* Adjusted margin for RTL */}
            </>
          ) : (
            <>
                  ...
                  {studentsInGrade.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-2 text-center flex flex-wrap justify-center gap-2">
                        <Link
                          to={`/students/${s.id}/details`} // Changed route to /students/:id/details for consistency
                        ...
                        <Link
                          to={`/edit-student/${s.id}`} // Changed route to /edit-student/:id for clarity
